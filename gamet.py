        ...
        if base_url in b:
            ...
            for p in paragraflar:
                ...
                if (turkcemi.turkcemi(p.text, fout=outfile) == True) and (len(p.text)>=1000):
                    ...
                    print("{} {:06d} {}".format(damga(), sayfasay,b), flush=True)
                    print("{} {:06d} {}".format(damga(),sayfasay,b),file=outfile, flush=True)
                    txttest = TXTDerlemTRText(p.text)
                    print("{} Toplam çalışma süresi = {} saniye".format(damga(),time.perf_counter()-basla),flush=True)
                    print("{} Toplam çalışma süresi = {} saniye".format(damga(),time.perf_counter()-basla),file=logfile,flush=True)
                else:
                    print("Bu metin Türkçe değildir veya yeterli sayıda geçerli Türkçe sözcük barındırmamaktadır.")
                    print("Bu metin Türkçe değildir veya yeterli sayıda geçerli Türkçe sözcük barındırmamaktadır.",file=logfile,flush=True)
